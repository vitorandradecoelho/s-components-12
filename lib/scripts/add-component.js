...
async function createComponentTemplate(componentName, componentPath, options = {}) {
  ...
  // Detecta se é lib ou src baseado no caminho
  const isLibComponent = componentPath.includes(path.join('lib', 'src', 'components'));
  const utilsPath = isLibComponent ? '../lib/utils' : '@/lib/utils';
...
async function main() {
  ...
  if (shouldCreate) {
    ...
    const libCreated = await createComponentTemplate(componentName, libComponentPath, options);
    const srcCreated = await createComponentTemplate(componentName, srcComponentPath, options);
    
    if (!libCreated || !srcCreated) {
      log('❌ Erro ao criar os componentes. Abortando...', 'red');
      process.exit(1);
    }
    log('');
  }
  ...
  const libComponentExists = fs.existsSync(libComponentPath);
  const srcComponentExists = fs.existsSync(srcComponentPath);
  
  if (!libComponentExists && !shouldCreate) {
    log(`⚠ Componente não encontrado em ${libComponentPath}`, 'yellow');
    log('   Use --create para criar o componente automaticamente', 'blue');
    log('');
  }
  
  if (!srcComponentExists && !shouldCreate) {
    log(`⚠ Componente não encontrado em ${srcComponentPath}`, 'yellow');
    log('   Use --create para criar o componente automaticamente', 'blue');
    log('');
  }
  
  // Se os componentes não existem, não atualizar os index
  if (!libComponentExists || !srcComponentExists) {
    log('❌ Um ou ambos os componentes não existem. Não é possível atualizar os index.', 'red');
    log('   Execute novamente com --create para criar os componentes.', 'blue');
    process.exit(1);
  }
  
  // Extrair exports do componente
  const exports = extractExports(libComponentPath);
